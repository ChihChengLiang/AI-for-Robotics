...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    L=len(newpath)
    TotalChange=tolerance+1
    while(TotalChange>tolerance):
        TotalChange=0
        for i in range(L):
            for dim in range(len(newpath[i])):
                change_data=weight_data*(path[i][dim]-newpath[i][dim])
                change_smooth=weight_smooth*(newpath[(i+1)%L][dim]+newpath[(i-1)%L][dim]-2*newpath[i][dim])
                newpath[i][dim]+=change_data
                newpath[i][dim]+=change_smooth
                TotalChange+=abs(change_data+change_smooth)
